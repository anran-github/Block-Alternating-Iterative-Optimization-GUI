...
def generate_best_grid_points(bounds, cost_expr, constraint_exprs, S, problem_data, max_candidates=20000):
    import numpy as np
...
def solve_from_initial_point(x0, cost_expr, constraint_exprs, bounds, mode="SLSQP"):
    ...
    if mode == "Block-Alternating Iter":
        result = minimize(cost_fn, x0, method='SLSQP', bounds=bounds, constraints=cons,options={'maxiter': 200})
    else:
...
def optimize_point_worker(init_pt_x, n, bounds, cost_expr, constraint_list, problem_data,
    ...
    """
    Block-Alternating Optimization:
        1) Fix P, optimize u
        2) Fix u, optimize P
    """
    ...
    def build_constraints(x_template, update_indices):
        """
        Build constraint functions where only selected indices are updated.
        """
        cons = []

        for expr in constraint_list:
            transformed = parse_constraint_expression(expr)

            def make_fun(trans_expr, idxs):
                return lambda var, trans_expr=trans_expr, idxs=idxs: float(
                    evaluate_expression(
                        trans_expr,
                        np.array([
                            var[0] if i == idxs[0] and len(idxs) == 1 else
                            var[i - idxs[0]] if len(idxs) > 1 and i in idxs else
                            x_template[i]
            ...
            cons.append({
                "type": "ineq",
                "fun": make_fun(transformed, update_indices)
            })
    ...
    for k in range(max_iters):
        ...
        def cost_u(u_var):
            x_full = pt.copy()
            x_full[0] = u_var[0]
            return float(evaluate_expression(cost_expr, x_full, problem_data))
        ...
        for expr in constraint_list:
            ...
            def make_fun(trans_expr):
                return lambda u_var, trans_expr=trans_expr: float(
                    evaluate_expression(
                        trans_expr,
                        np.array([u_var[0], pt[1], pt[2], pt[3]]),
                        problem_data
                    )
                )

            cons_u.append({"type": "ineq", "fun": make_fun(transformed)})
        ...
        for expr in constraint_list:
            ...
            def make_fun(trans_expr):
                return lambda P_var, trans_expr=trans_expr: float(
                    evaluate_expression(
                        trans_expr,
                        np.array([pt[0], P_var[0], P_var[1], P_var[2]]),
                        problem_data
                    )
                )

            cons_P.append({"type": "ineq", "fun": make_fun(transformed)})
        ...
        if res_P.success:
            pt[1:] = res_P.x.astype(float)
            results.append(res_P)

        # =========================
        # stopping criterion
        # =========================
        if np.linalg.norm(pt - pt_old) < tol:
            break
...
class OptimizationApp(QWidget):
    def __init__(self):
        ...
        self.num_init_input = QLineEdit("200")
        ...
        self.algo_selector.addItems(["Block-Alternating Iter", "SLSQP", "COBYLA","trust-constr","Interior-Point", "GA", "Particle Swarm"])
    ...
    def _solve_with_Multi_SCD(self, n, S, bounds, cost_expr, constraint_list, tol=1e-4, max_iters=50):
        ...
        self.result_output.append("Running Block-Alternating Iteration Optimization (Sequential)...\n")
        ...
        initial_points = generate_best_grid_points(
            bounds, cost_expr, constraint_list, S, problem_data=self.problem_data
        )
        print(f"Generated {len(initial_points)} initial points using LHS.")
        cost_results = []
        all_results_buffer = []
        diff = tol + 1
        iter_count = 0
        ...
        while diff > tol and iter_count < max_iters:
            ...
            for init_pt_x in initial_points:
                pt, res_list = optimize_point_worker(
        ...
        for res_list in all_results:
            ...
            valid_costs = [r.fun for r in res_list if r.success]
            if len(valid_costs) == 0:
                min_cost.append(float('inf'))
            else:
                min_cost.append(min(valid_costs))
        ...
        # =========================
        # Cost trace for plotting
        # =========================
        cost_results_plot = []
        for iter_block in all_results_buffer:
            if len(iter_block) == len(initial_points):
                cost_results_plot.extend([x.fun for x in iter_block[lowest_idx] if hasattr(x, "fun")])

        self._plot_cost_trace(cost_results_plot, method="Multi_SCD")

        print(f"Cost Value: {best.fun:.6g}, time: {t_end - t_start:.4f} s, "
            f"Best solution: {best.x}, iterations: {len(cost_results_plot)}")
    ...
    def _solve_with_GA(self, n, num_init, bounds, cost_expr, constraint_list,
        ...
        print(f"Cost Value: {best_cost:.12g}, time: {t_end - t_start:.4f} s, iterations: {len(cost_trace)}")
    ...
    def _solve_with_PSO(self, n, num_init, bounds, cost_expr, constraint_list,
        ...
        print(f"Cost Value: {gbest_cost:.12g}, time: {t_end - t_start:.4f} s, iterations: {len(cost_trace)}")


    def _solve_with_interior_point(self, n, num_init, bounds, cost_expr, constraint_list):
        self.result_output.clear()
        self.result_output.append("Running Interior-Point Optimization (Multi-start)...\n")
        ...
        for idx, x0 in enumerate(initial_points):
            ...
            for expr in constraint_list:
                ...
                def make_fun(trans_expr):
                    def fun(x):
                        ...
                        try:
                            val = eval(trans_expr, {"__builtins__": {}}, env)
                            return float(np.asarray(val).squeeze())
                        except Exception:
                            return -1e10
            ...
            for expr in constraint_list:
                ...
                try:
                    u, P, xt, x_next, xbar, ubar, A, B = unpack_vars(x_sol, self.problem_data)
                    ...
                    val = eval(transformed, {"__builtins__": {}}, env)

                    if np.any(np.asarray(val) < -1e-6):  # tolerance
                        feasible = False
                        break
            ...
            if feasible and cost_val < best_cost:
                ...
                best_trace = cost_trace

            # print per run
            self.result_output.append(
                f"Init {idx+1}: cost = {cost_val:.6g}, "
                f"feasible = {feasible}, success = {res.success}, "
        ...
        self.result_output.append(f"Total time cost: {t_total_end - t_total_start:.4f} seconds")

        # =========================
        # Optional: summary statistics
        # =========================
        avg_time = np.mean([s["time"] for s in summary_table])
        avg_cost = np.mean([s["cost"] for s in summary_table])

        self.result_output.append("\n=== Summary ===")
        self.result_output.append(f"Average cost: {avg_cost:.6g}")
        self.result_output.append(f"Average time per run: {avg_time:.4f} s")

        if best_trace:
            self._plot_cost_trace(best_trace, method="Interior-Point")
   

        print(f"Cost Value: {best_cost:.12g}, time: {t_total_end - t_total_start:.4f} s, iterations: {len(cost_trace)}")
    ...
    def solve_optimization(self):
        ...
        if algo in ["SLSQP", "COBYLA", "trust-constr"]:
            self._solve_with_other_inernal_opt(
                n, num_init, bounds, cost_expr, constraint_list, algo
            )

        elif algo == "Interior-Point":
            self._solve_with_interior_point(
                n, num_init, bounds, cost_expr, constraint_list
            )
